...
export function HeroBanner({
  ...
  const handleClick = () => {
    ...
    window.open(onClick, "_blank"); // Opens the URL in a new tab
  };
    ...
    <div className={cn("mx-auto flex items-center justify-center", className)}>
      <AnimatePresence>
        ...
        >
          ...
          >
            <SettingsFilled className="text-[#14cdc8]" />
          </motion.div>
          <motion.div
            initial="hidden"
            animate={isHovered ? "visible" : "hidden"}
          ...
          <div
            className="relative flex h-[35px] items-center gap-1 rounded-[6px] border border-[#CBE7FF] bg-[#E6F9F9] pl-2.5 pr-1 text-sm"
            style={{ backgroundColor: "#E6F9F9" }} // Lighter shade of #14cdc8
          >
            <button
              className="focus-visible:shadow-focus-ring rounded-xs my-[-1px] cursor-pointer border-none bg-transparent px-0 py-1 font-sans text-[13px] font-medium text-[#14cdc8] underline decoration-[#CAE7FF] underline-offset-[5px] outline-none hover:text-[#14cdc8] hover:decoration-[#94CCFF] focus-visible:!shadow-[#008FFF]"
            ...
            <span className="text-[0.8125rem] text-[#14cdc8]">
              {description}
            </span>
          </div>
        </motion.div>
